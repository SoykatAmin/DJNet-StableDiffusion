...
Args:
source_a_audio: Original source A audio
transition_audio: Generated transition audio
source_b_audio: Original source B audio
source_a_spec: Source A spectrogram (optional)
transition_spec: Transition spectrogram (optional)
source_b_spec: Source B spectrogram (optional)
...
source_a_audio, transition_audio, source_b_audio, metrics, output_path
)
...
# Use high-pass filter to estimate noise floor
...
# Spectral similarity between transition and sources
# Use mel spectrograms for comparison
...
# Transition novelty (how different is transition from average of sources)
...
# Create continuous sequence: A -> Transition -> B
# Take last portion of A, full transition, first portion of B
segment_length = len(transition) // 4 # Use 1/4 of transition length
...
# 1. RMS variation
window_size = len(transition) // 20 # Small window
rms_values = []
...
"""Analyze musical features like tempo, key, etc."""
metrics = {}
...
metrics[f'{name}_tempo'] = float(tempo)
except:
...
"""Calculate perceptual quality metrics from spectrograms"""
metrics = {}
...
# Compare transition to interpolated sources
interpolated = (source_a_spec + source_b_spec) / 2
ssim_score = ssim(transition_spec, interpolated, data_range=2.0) # data range is [-1, 1]
metrics['structural_similarity_to_interpolation'] = ssim_score

# Gradient similarity (edge preservation)
